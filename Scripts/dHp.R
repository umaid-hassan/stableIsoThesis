...
library(ncdf4)
precip <- nc_open("../data/NUDGING_ERA5_T127L95_v1.2_echam6.d2Hp.20000101-20031231.daymean.nc")
...
obsoutput <- ncvar_get(precip, varid = 'd2Hp',
                       start= c(clon, clat,1),
                       count = c(1,1,-1)) #count '-1' means 'all values along that dimension'that dimension'
# create dataframe
datafinal <- data.frame(date= precipdatadates, d2Hp = obsoutput, lat = clat, lon =clon)
head(datafinal)
...
addWorksheet(wb, "d2Hp")
writeData(wb, sheet = "d2Hp", datafinal, headerStyle = header_style,rowNames = FALSE)

freezePane(wb, sheet = "d2Hp", firstRow = TRUE)
