...
precip <- nc_open("../data/NUDGING_ERA5_T127L95_v1.2_echam6.aprt.20000101-20031231.daymean.nc")
...
lat <- 39.9334 # latitude  of location
...
obsoutput <- ncvar_get(precip, varid = 'aprt',
                       start= c(clon, clat,1),
                       count = c(1,1,-1)) #count '-1' means 'all values along that dimension'that dimension'
# create dataframe
datafinal <- data.frame(date= precipdatadates, aprt = obsoutput, lat = clat, lon =clon)
head(datafinal)
...
writeData(wb, "Precip", datafinal, headerStyle = header_style)
freezePane(wb, "Precip", firstRow = TRUE)
setColWidths(wb, "Precip", cols = 1:ncol(datafinal), widths = "auto")
saveWorkbook(wb, file = "Lisbon.xlsx", overwrite = TRUE)
