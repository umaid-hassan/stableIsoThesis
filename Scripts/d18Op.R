...
library(ncdf4)
precip <- nc_open("../data/NUDGING_ERA5_T127L95_v1.2_echam6.d18Op.20000101-20031231.daymean.nc")
...
obsoutput <- ncvar_get(precip, varid = 'd18Op',
                       start= c(clon, clat,1),
                       count = c(1,1,-1)) #count '-1' means 'all values along that dimension'that dimension'
# create dataframe
datafinal <- data.frame(date= precipdatadates, d18Op = obsoutput, lat = clat, lon =clon)
head(datafinal)
...
addWorksheet(wb, "d18Op")
writeData(wb, sheet = "d18Op", datafinal, headerStyle = header_style,rowNames = FALSE)

freezePane(wb, sheet = "d18Op", firstRow = TRUE)
